...
MessageDlg3("This is the clasquinator demo", "Ok")

ShowIntro()

name$ = InputDlg3$("Let's get acquainted. What is your name?", "Who are you?", "OK", "Cancel")

welcomemessage$ = "Welcome, " + name$ + ". Let's get this show on the road!"
MessageDlg3(welcomemessage$, "Ok")

LikeIt()

//TextFileDlg3("README.md", "The README file", "OK")

name$ = PasswordDlg3$("Please enter your banking password", "Banking password?", "OK","Cancel")

MessageDlg3("KIDDING! I was just kidding!", "Haha")

name$= "But for the record, you entered: " + name$
MessageDlg3(name$, "Ok")

MessageDlg3("I'm bored. Let's change the colour scheme", "Change!")
ChangeColour("white", "green", "yellow")

name$ ="Remember that clunky text menu at the start?"
MessageDlg3(name$, "Yeah")
while(a$ <> "q")
	a$ = ShowMenu$()
wend

//MessageDlg3("Unfortunately, clasquinator has no calendar widget ATM", "Ok")
MessageDlg3("You can select a directory ...", "OK")
a$ = GetDirDlg3$("Pick a folder (case-sensitive!)", "Which directory would you like to choose?", "..")
MessageDlg3("You chose " + a$ + ". Good choice!", "OK")
if peek$("os") = "unix" then
	b$ = GetFileDlg3$("Pick a file (case-sensitive!)", "Now let's pick a file in that directory","../" + a$)
elseif peek$("os") = "windows"
	b$ = GetFileDlg3$("Pick a file (case-sensitive!)", "Now let's pick a file in that directory","..\\" + a$)
endif

MessageDlg3("You chose " + b$ + " in "+ a$ + ". Another good choice!", "OK")

ChangeColour("cyan", "blue", "red")

WaitDlg3(1,"Thank you for playing!")
pause 4 :WaitDlg3(0)
if peek$("os") = "unix" then
...
sub ShowIntro()
	...
	wend
	 StringDisplayDlg3(a$, "Welcome!", "Ok")
end sub

sub ShowMenu$()
local a$, b$, c
a$ =MenuDlg3$("Main Menu", "Graphical#Textual#Quit", 0)
switch a$
	case "g"
		b$ =MenuDlg3$("Graphical Menu", "Kdialog#Zenity", 1, "Graphical#Textual#Quit")
		switch b$
			case "k"
				MessageDlg3("You chose Kdialog", "OK")
				break
			case "z"
				MessageDlg3("You chose Zenity", "OK")
				break
		end switch
		break
	case "t"
		b$ =SubMenuDlg3$("Textual Menu", "Clasquinator#Dialog#Whiptail", "Graphical#Textual#Quit")
		switch b$
			case "c"
				MessageDlg3("You chose Clasquinator", "OK")
				break
			case "d"
				MessageDlg3("You chose Dialog", "OK")
				break
			case "w"
				MessageDlg3("You chose Whiptail", "OK")
				break
		end switch
		break
	default
		c = YesNoDlg3("Quitting the menu demo so soon?", "Yes", "No")
		if c = 0 a$ = ""
		break
end switch
return a$
...
sub LikeIt()
	if YesNoDlg3("Do you like what you are seeing so far?", "Yes", "No") then
		MessageDlg3("Cool! Let's throw up a text file.", "ok")
	else
		MessageDlg3("Fair enough. But let's throw up a text file.", "Ok")
	endif
	TextFileDlg3("pg4.txt", "The Gettysburg Address", "OK")
end sub
