...
## pre-alpha and very unstable - give me some more time with this!
...
+ **ClearDlg**() - on dialog and whiptail, this clears the dialog from your terminal. Same as the yabasic command CLEAR SCREEN, but may work better if you are expecting to issue another Linux command immediately. On yad and kdialog, dummy routines are provided for compatibility so that you do not need to rewrite your code.
...
+ **InputDlg$**(text$, title$, ok$, cancel$) - Presents a one-line dialog into which the user can type a string answer.
    + The value *title$* has no effect in yad but is kept here for compatibility and must be supplied.
...
+ **MenuDlg**(text$, title$, ok$, cancel$, menustring$) - Create a menu of options for the user to choose from
   ...
   + Please note that the height of this widget is hardcoded at 400 pixels, because it looked really crap when allowed to autosize. This may change with future releases of yad.
...
+ **MessageDlg**(text$, ok$) - display a simple message with an OK button.
    + The value *ok$* has no effect in yad, but is kept here for compatibility. It can be omitted.
    + Returns nothing.
   + *Example:*
```
MessageDlg(EncloseText$("This is a simple message dialog.\nClose it with ENTER or try clicking on OK."),"OK")
```
...
+ **RadioDlg**(text$, title$, ok$, cancel$, menustring$, selected$) - Create a menu of options for the user to choose from
    ...
    + The value *cancel$* is the text of the reject button, normally Cancel.
    + The value *menustring$* is the list of menu options as a single string, separated by hash signs (#). Spaces are allowed.
    + The value *selected$* has no effect in yad, but is kept here for compatibility and must be supplied.
    + If the number of options exceed the available space in the widget, the list will be scrollable.
    + The result is returned as a numeric value, starting from 1.
...
+ **TextFileDlg**(filename$, title$, exit$) - Displays a text file.
    + The value *title$* has no effect on yad, but is used here for compatibility reasons and must be supplied.
...
+ **YesNoDlg**(text$,yes$, no$) - Displays a dialog with text (normally a question) and the options to reply yes or no.
    + The value *yes$* and *no$* have no effect on yad, but are kept here for compatibility reasons. They can be omitted.
...
+ **YCalendarDlg$**(text$, ok$, cancel$) - Display a calendar widget with today's date pre-selected.
    ...
    + The value *ok$* is the text of the accept button, normally OK.
    + The value *cancel$* is the text of the reject button, normally Cancel.
    + The value *text$* has no effect on yad, but is used here for compatibility reasons and must be supplied.
    + On exit, the date is returned in the form dd/mm/yyy, e.g. 23/05/2018
    + ONLY available in *yad* - for *dialog* and *kdialog*, see *DCalendarDlg$()* and *KCalendarDlg$()*
    + *DCalendarDlg$* and *ZCalendarDlg$* are fully compatible.
    + *EXAMPLE: print ZCalendarDlg$("\"Pick a date\"", "Pick", "Escape")*

![YCalendarDlg](./imgs/YCalendarDlg.png)
...
spaces in it.
    + ONLY available in *yad*, but *kdialog* and *zenity* have comparable set of commands.
...
+ **YNotifyDlg**(text$) - pop up a GTK notification.
    ...
    + ONLY available in *yad* (*zenity* has something similar). If you need a general utility, try *NotifyDlg()* in the *linuxmisclib* library.
...
+ **YOpenFileDlg$**() - Select a file to open.
 + Result is returned as a string value.
    ...
    + *Example:*
```
a$ = StripLFoffSysCmd$(YSaveFileDlg$())
``` 
![YOpenFileDlg](imgs/YOpenFileDlg.png)
